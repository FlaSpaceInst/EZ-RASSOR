...
class ImageHolder:
    def __init__(self):
        ...
        self.location_pub = rospy.Publisher('object_detection/paver_location', Float64MultiArray, queue_size=10)
    
    def SetImage(self, data):
        self.image = self.cv_bridge.imgmsg_to_cv2(data, desired_encoding='passthrough')
    ...
    def ObjectDetection(self, data):
        ...
        PATH_TO_FIG = rospack.get_path('ezrassor_arm_autonomous_control') + '/source/ezrassor_arm_autonomous_control/frozen_inference_graph.pb'
        PATH_TO_TXT = rospack.get_path('ezrassor_arm_autonomous_control') + '/source/ezrassor_arm_autonomous_control/faster_rcnn_inception_v2_coco_2018_01_28.pbtxt'
        ...
        cv2.rectangle(color_image, (int(left), int(top)), (int(right), int(bottom)), (0, 255, 0), thickness=2)
        ...
        cv2.putText(color_frame, "{}mm".format(distance), (pointX, pointY), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
        
        #Convert our pixel coordinates to physical cartesian coordinates
        (xCoord, yCoord, zCoord) = self.ConvertToPhysCoords(pointX, pointY, distance, self.GetInfo())
...
def onStart(camera_image_topic, camera_info_topic, command_topic):
    try:
        ...
        holder = ImageHolder()
        rospy.Subscriber('/ezrassor1/color/image_raw_arm', Image , holder.SetImage)
        rospy.Subscriber('/ezrassor1/depth/image_raw_arm', Image, holder.SetDepthImage)
        rospy.Subscriber('/ezrassor1/color/camera_info_arm', CameraInfo, holder.SetInfo)
