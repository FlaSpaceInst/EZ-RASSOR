...
class ArmAutoHelper:
    ...
    def __init__(
        ...
        rospy.Subscriber(
            "/gazebo/model_states", ModelStates, self.setModelStates
        )
        ...
        rospy.Subscriber("depth/points", PointCloud2, callback=None)
        ...
        self.arm_pub = rospy.Publisher(
            "/ezrassor1/paver_arm_controller_instructions",
            Float64MultiArray,
            queue_size=10,
        )
        self.object_detection_pub = rospy.Publisher(
            "/ezrassor1/paver_arm_object_detection_command_instructions",
            Float32,
            queue_size=10,
        )
    ...
    def moveHomeCallback(self, data):
        self.returnToSpawnPrep(
            [self.model_poses.position.x, self.model_poses.position.y]
        )
        self.auto_drive_location(
    ...
    def getRadOrientationOfRover(self):
        return euler_from_quaternion(
            [
                self.model_poses.orientation.x,
                self.model_poses.orientation.y,
                self.model_poses.orientation.z,
                self.model_poses.orientation.w,
            ]
        )
    ...
    def spawnPaver(self):
        ...
        data = [
            1,
            round(self.model_poses.position.x, 5) - 0.07,
            round(self.model_poses.position.y, 5) + 0.04,
            round(self.model_poses.position.z, 5) + 0.13,
            yaw - 1.54,
        ]
        rospy.loginfo(data)
        # data = [data.data[0], ]
        ...
        output = "screen"
        args = "-database paver_{} -sdf -model paver{} -x {} -y {} -z {} -Y {}".format(
            self.paver_tracker,
            self.paver_count,
            data[1],
            data[2],
        ...
        node = roslaunch.core.Node(
            package=package, node_type=node_type, output=output, args=args
        )
    ...
    def spawnPaverAtTarget(self, target):
        ...
        node_type = "spawn_model"
        output = "screen"
        args = "-database paver -sdf -model paver_test -x {} -y {} -z {} -Y {}".format(
    ...
    def full_auto(self):
        ...
        while True:
            ...
            self.last_paver = [
                self.model_poses.position.x + 1.0,
                self.model_poses.position.y + 0.3,
            ]
