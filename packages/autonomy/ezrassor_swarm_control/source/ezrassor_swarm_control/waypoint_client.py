...
class WaypointClient:
    def __init__(self, robot_num):
        self.name = '/ezrassor{}/waypoint'.format(robot_num)

        # Create the waypoint action client
        self.client = actionlib.SimpleActionClient(self.name, waypointAction)
    ...
    def send_path(self, data):
        ...
        rospy.loginfo('Action client {} received path from {} to {}'.
                      format(self.name, (path[0].x, path[0].y), (path[-1].x, path[-1].y)))

        for node in path:
            self.send_waypoint(node)

        rospy.loginfo('Action client {} succeeded, path completed!'.format(self.name))
