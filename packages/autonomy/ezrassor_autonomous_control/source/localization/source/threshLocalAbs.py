...
def threshLocalAbs(img, winSiz=9):
        ...
        while col <= cols:
            ...
            while r <= sr:
                ...
                while c <= sc:
                    ...
                    c = c + 1
                r = r + 1
            windowInt = windowInt - img.item(row, col)
            avgWinInt = float(windowInt) / winSiz2
            absVal = img.item(row, col) - avgWinInt
            thrsh.itemset((row, col), absVal)
            # update max abs
            if absVal > maxAbsVal:
                maxAbsVal = absVal
            col = col + 1
        row = row + 1
    threshVal = maxAbsVal / magic
