...
class Star_Cat:
    ...
    def __init__(self, image_file, angle_size, fov, scope_size):
        ...
        while True:
            ...
            ra = ra_to_decimal_deg(float(star_splits[3]),
                          float(star_splits[4]),
                          float(star_splits[5]))
            # Covert DEC to decimal
            dec = dec_to_decimal_deg(float(star_splits[6]),
                            float(star_splits[7]),
                            float(star_splits[8]))
            # Create star_ref
            ...
            for sid_starB in self.adj_star_list:
                if sid_starA == sid_starB:
                    ...
                    if angular_distance < fov:
                        self.adj_star_list[sid_starA].add(sid_starB,
                                                          angular_distance)
                        elem = int(angular_distance * (1 / angle_size))
                        if elem == 0:
                            self.angles_list[elem].update([sid_starA,
                                                           sid_starB])
                            self.angles_list[elem + 1].update([sid_starA,
                                                               sid_starB])
                        else:
                            self.angles_list[elem - 1].update([sid_starA,
                                                               sid_starB])
        ...
        for sid_star in self.adj_star_list:
            ...
            angular_distance = calculate_angular_distance(ra, dec,
                                                          init_ra_position,
                                                          init_dec_position)
            if angular_distance < (hfov + scope_size):
                self.scope_adj_star_list[sid_star] = Star_Ref(sid_star,
        ...
        for sid_starA in self.scope_adj_star_list:
            for sid_starB in self.scope_adj_star_list:
                if sid_starA == sid_starB:
                    ...
                    if angular_distance < fov:
                        self.scope_adj_star_list[sid_starA].add(sid_starB,
                                                              angular_distance)
                        elem = int(angular_distance * (1 / angle_size))
                        if elem == 0:
                            self.scope_angles_list[elem].update([sid_starA,
                                                                 sid_starB])
                            self.scope_angles_list[elem + 1].update([sid_starA,
                                                                    sid_starB])
                        else:
                            self.scope_angles_list[elem - 1].update([sid_starA,
                                                                    sid_starB])
    ...
    def rescope(self, ra_position, dec_position): # in degrees
        angular_distance = calculate_angular_distance(ra_position,
                                                      dec_position,
                                                      self.scope_position_ra,
                                                      self.scope_position_dec)
        ...
        for sid_star in self.adj_star_list:
            ra = self.adj_star_list[sid_star].get_ra()
            dec = self.adj_star_list[sid_star].get_dec()
            angular_distance = calculate_angular_distance(ra, dec, ra_position,
                                                          dec_position)
    ...
    def match_global(self, stars): # 5 stars ordered brightest to least
        return match(stars, self.angle_size, self.num_stars,
                     self.angle_size, self.angles_list, self.adj_star_list)
        # end

    def match_scope(self, stars): # 5 star objects order brightest to less
        list_of_possible_matches = match(stars, self.angle_size,
                                         self.num_stars, self.angle_size,
                                         self.angles_list,
                                         self.scope_adj_star_list)
...
def match(stars, epsilon, num_stars, angle_size, angles_list, adj_star_list):
    ...
    for combo in candidate_temp_permutations:
        ...
        distance12 = calculate_angular_distance(
                                             ...
                                             adj_star_list[combo[2]].get_ra(),
                                             adj_star_list[combo[2]].get_dec())
        if (distance02 < (angular_distances[(0,2)] + epsilon) and
            distance02 > (angular_distances[(0,2)] - epsilon)):
    ...
    for combo in candidate_temp_temp_permutations:
        for x in candidates[3]:
            if combo[0] != x and combo[1] != x and combo[2] != x:
                candidate_temp_permutations.append((combo[0],
                                                    combo[1], combo[2], x))
    for combo in candidate_temp_permutations:
        ...
        distance23 = calculate_angular_distance(
                                             ...
                                             adj_star_list[combo[3]].get_ra(),
                                             adj_star_list[combo[3]].get_dec())
        if (distance03 < (angular_distances[(0,3)] + epsilon) and
            distance03 > (angular_distances[(0,3)] - epsilon)):
            if (distance13 < (angular_distances[(1,3)] + epsilon) and
                distance13 > (angular_distances[(1,3)] - epsilon)):
    ...
    for combo in candidate_temp_temp_permutations:
        for x in candidates[4]:
            if (combo[0] != x and combo[1] != x and combo[2] != x and
                                                    combo[3] != x):
                candidate_temp_permutations.append((combo[0], combo[1],
                                                    combo[2], combo[3], x))
    for combo in candidate_temp_permutations:
        ...
        distance34 = calculate_angular_distance(
                                             ...
                                             adj_star_list[combo[4]].get_ra(),
                                             adj_star_list[combo[4]].get_dec())
        if (distance04 < (angular_distances[(0,4)] + epsilon) and
            distance04 > (angular_distances[(0,4)] - epsilon)):
            if (distance14 < (angular_distances[(1,4)] + epsilon) and
                distance14 > (angular_distances[(1,4)] - epsilon)):
                if (distance24 < (angular_distances[(2,4)] + epsilon) and
                    distance24 > (angular_distances[(2,4)] - epsilon)):
    ...
    candidate_permutations = []
    for combo in candidate_temp_permutations:
        candidate_permutations.append(( (combo[0],
